...
$(kernel_object_files): build/kernel/%.o : src/%.c
	mkdir -p $(dir $@) && \
	x86_64-elf-gcc -c -I src/include -ffreestanding $(patsubst build/%.o, src/%.c, $@) -o $@
...
build-x86_64: $(kernel_object_files)
	mkdir -p dist/x86_64 && \
	x86_64-elf-ld -n -o dist/x86_64/sirius.bin -T ld/link.ld $(kernel_object_files) && \
	cp dist/x86_64/sirius.bin sysroot/boot/sirius.bin && \
	grub-mkrescue /usr/lib/grub/i386-pc -o dist/x86_64/SiriusOS.iso sysroot