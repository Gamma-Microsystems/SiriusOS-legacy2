...
build-x86_64: $(kernel_object_files)
	...
	mkfs -t fat SiriusOS
	syslinux SiriusOS
	$(ROOTCMD) mount SiriusOS iso_root
	$(ROOTCMD) mkdir iso_root/boot
	$(ROOTCMD) cp sysroot/boot/sirius.bin sysroot/boot/syslinux.cfg iso_root/boot
	$(ROOTCMD) umount iso_root